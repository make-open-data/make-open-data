...
with commune_data as (
    select * from {{ ref('Logement_Caracteristiques_communes_evolution') }}
)
...
select 
    ...
    nom_region,
    annee
    {%- set columns = adapter.get_columns_in_relation(ref('Logement_Caracteristiques_communes_evolution')) %}
    {%- for col in columns %}
